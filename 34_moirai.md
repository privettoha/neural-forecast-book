...
Интересно отметить, что идея multi-patch projection концептуально близка к архитектуре Mixture of Experts (MoE), которая активно используется в современных языковых моделях вроде Mixtral. В MoE разные «эксперты» специализируются на разных типах входных данных, и маршрутизатор выбирает, какого эксперта использовать для каждого токена. В Moirai аналогичную роль играют разные projection layers, а «маршрутизатором» служит частота данных.
...
Технически это реализуется через Rotary Positional Embeddings (RoPE), которые кодируют позиционную информацию путём поворота векторов в пространстве эмбеддингов. В Moirai RoPE применяется отдельно для временной позиции и для индекса переменной, что обеспечивает два важных свойства:
...
LOTSA является открытым датасетом и доступен на HuggingFace, что делает возможным воспроизведение результатов и дальнейшие исследования в области фундаментальных моделей временных рядов.

## Практическое применение Moirai

Установка и использование Moirai осуществляется через библиотеку uni2ts, разработанную командой Salesforce:
...
Moirai интегрирована с GluonTS — популярной библиотекой для прогнозирования временных рядов от AWS, что упрощает включение модели в существующие пайплайны.
