...
В общем виде encoder трансформирует входную последовательность x=(x1,x2,...,xT)\mathbf{x} = (x_1, x_2, ..., x_T) x=(x1​,x2​,...,xT​) в последовательность скрытых представлений:

H(l)=TransformerBlock(H(l−1))\mathbf{H}^{(l)} = \text{TransformerBlock}(\mathbf{H}^{(l-1)})H(l)=TransformerBlock(H(l−1))
...
ri=∣yi−y^i∣r_i = |y_i - \hat{y}_i|ri​=∣yi​−y^​i​∣

где yiy_i yi​ — истинное значение, y^i\hat{y}_i y^​i​ — прогноз модели.

Затем для нового прогноза мы строим интервал, используя квантиль распределения ошибок. Для уровня доверия (1−α)(1 - \alpha) (1−α) интервал имеет вид:

[y^−q1−α,y^+q1−α][\hat{y} - q_{1-\alpha}, \hat{y} + q_{1-\alpha}][y^​−q1−α​,y^​+q1−α​]

где q1−αq_{1-\alpha} q1−α​ — (1−α)(1-\alpha) (1−α)-квантиль эмпирического распределения ошибок {r1,r2,...,rn}\{r_1, r_2, ..., r_n\} {r1​,r2​,...,rn​}.

Красота conformal prediction в том, что он даёт теоретическую гарантию: если данные exchangeable (грубо говоря, будущее статистически похоже на прошлое), то интервал покроет истинное значение с вероятностью не меньше (1−α)(1-\alpha) (1−α). Это сильнее, чем эвристические интервалы многих других моделей.
