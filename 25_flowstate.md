...
SSM — это класс моделей, который пришёл из теории управления и обработки сигналов. В контексте глубокого обучения SSM стали популярны благодаря работам [S4](https://arxiv.org/abs/2111.00396) (2021) и [Mamba](https://arxiv.org/abs/2312.00752) (2023), которые показали, что SSM могут конкурировать с трансформерами на задачах моделирования последовательностей.
...
где $h(t)$ — скрытое состояние, $x(t)$ — вход, $y(t)$ — выход, а $A$, $B$, $C$, $D$ — обучаемые матрицы.
...
**Рекуррентный режим** — как RNN, шаг за шагом. Эффективен для инференса: $O(1)$ память на шаг, можно обрабатывать бесконечные потоки данных.
...
Вторая ключевая инновация FlowState — Functional Basis Decoder (FBD). Вместо того чтобы предсказывать дискретные значения $\hat{y}_1, \hat{y}_2, ..., \hat{y}_H$, модель предсказывает коэффициенты разложения по базисным функциям:

$$\hat{y}(t) = \sum_{k=1}^{K} c_k \cdot \phi_k(t)$$

где $\phi_k(t)$ — базисные функции (например, синусы и косинусы разных частот), $c_k$ — обучаемые коэффициенты, которые выдаёт decoder.

Это превращает прогноз из набора точек в непрерывную функцию. Хотите прогноз на 24 часа с шагом 15 минут? Вычислите $\hat{y}(t)$ для $t = 0.25, 0.5, 0.75, ..., 24$. Хотите тот же прогноз с шагом 1 час? Вычислите для $t = 1, 2, 3, ..., 24$. Модель та же, коэффициенты те же, меняется только точка вычисления.

Идея похожа на basis expansion в [N-BEATS](https://arxiv.org/abs/1905.10437), но там базис применялся к дискретному выходу. Здесь базис создаёт истинно непрерывное представление.
...
$$\text{scale\_factor} = \frac{\text{базовая сезонность}}{\text{сезонность ваших данных}} = \frac{24}{N}$$
...
$$\phi_k^{\text{poly}}(t) = t^k, \quad k = 0, 1, 2, ...$$

**Гармонический базис** — для сезонности:

$$\phi_k^{\text{sin}}(t) = \sin(2\pi k t / P)$$
$$\phi_k^{\text{cos}}(t) = \cos(2\pi k t / P)$$

где $P$ — период базовой сезонности.
