...
В стандартном трансформере для временных рядов входная матрица имеет размер (T,C)(T, C) (T,C) — время × каналы. Каждый временной шаг становится токеном размерности CC C, и self-attention вычисляется между токенами, то есть между моментами времени.
...
**Квадратичная сложность по времени.** Матрица внимания имеет размер T×TT \times T T×T. Для длинных рядов (тысячи точек) это становится узким местом. PatchTST решает эту проблему через патчинг, но iTransformer предлагает альтернативу.

**Потеря индивидуальности переменных.** Когда все переменные в момент tt t сливаются в один токен, модели сложно выучить, что переменные ведут себя по-разному. Температура и влажность имеют разную динамику, разные автокорреляции, разные сезонности — но в традиционном подходе они смешиваются на входе.

## Инверсия: переменные как токены

iTransformer переворачивает логику. Вместо того чтобы делать токены из временных шагов, он делает токены из переменных. Входная матрица (T,C)(T, C) (T,C) транспонируется в (C,T)(C, T) (C,T), и теперь каждая строка — это полная история одной переменной.
...
Для каждой переменной ii i её история xi∈RTx_i \in \mathbb{R}^T xi​∈RT проецируется в пространство размерности DD D:

hi(0)=Embedding(xi)=We⋅xi+beh_i^{(0)} = \text{Embedding}(x_i) = W_e \cdot x_i + b_ehi(0)​=Embedding(xi​)=We​⋅xi​+be​

где We∈RD×TW_e \in \mathbb{R}^{D \times T} We​∈RD×T — матрица проекции. Это линейное преобразование, которое сжимает TT T временных точек в DD D-мерное представление.
...
После эмбеддинга у нас есть CC C токенов размерности DD D. К ним применяется стандартный transformer encoder, но теперь attention работает между переменными:

Attention(Q,K,V)=softmax(QKTdk)V\text{Attention}(Q, K, V) = \text{softmax}\left(\frac{QK^T}{\sqrt{d_k}}\right) VAttention(Q,K,V)=softmax(dk​​QKT​)V

где QQ Q, KK K, VV V — проекции токенов переменных. Матрица внимания имеет размер C×CC \times C C×C — количество переменных, а не временных шагов.

Feed-forward network применяется к каждому токену (переменной) независимо:

FFN(h)=W2⋅GELU(W1⋅h+b1)+b2\text{FFN}(h) = W_2 \cdot \text{GELU}(W_1 \cdot h + b_1) + b_2FFN(h)=W2​⋅GELU(W1​⋅h+b1​)+b2​

Эти FFN-слои — единственное место, где происходит «обработка» временной информации. Они преобразуют DD D-мерное представление истории переменной, потенциально выучивая временные паттерны, но без явного механизма attention по времени.

### Projection: от токена к прогнозу

После LL L слоёв encoder'а каждый токен (переменная) проецируется в прогноз:

y^i=Wp⋅hi(L)+bp\hat{y}_i = W_p \cdot h_i^{(L)} + b_py^​i​=Wp​⋅hi(L)​+bp​

где Wp∈RH×DW_p \in \mathbb{R}^{H \times D} Wp​∈RH×D — матрица проекции в горизонт HH H.
...
**Вычислительная эффективность.** Матрица внимания размера C×CC \times C C×C обычно гораздо меньше, чем T×TT \times T T×T. Если у вас 100 переменных и 1000 временных точек, iTransformer вычисляет 100×100=10,000100 \times 100 = 10{,}000 100×100=10,000 весов внимания вместо 1,000×1,000=1,000,0001{,}000 \times 1{,}000 = 1{,}000{,}000 1,000×1,000=1,000,000. Это снижение в 100 раз.
...
**Channel independence в PatchTST.** PatchTST обрабатывает каналы независимо и показывает, что это работает. iTransformer идёт дальше: вместо независимости он явно моделирует зависимости между каналами через attention, но при этом отказывается от temporal attention.

**Feature mixing в TSMixer.** TSMixer чередует time-mixing и feature-mixing через MLP. iTransformer можно рассматривать как экстремальный вариант, где feature-mixing делается через attention (более мощный механизм), а time-mixing — через FFN (более простой).
...
**Не для всех задач.** Если переменные действительно независимы, моделирование их связей — пустая трата ёмкости модели. Channel-independent подходы (PatchTST, N-HiTS) могут быть лучше.
...
|Много каналов (>100)|~ C2C^2 C2 attention|✓ Независимо|~ CC C mixing|
...
В следующей части мы обратимся к архитектурам, которые многие считали устаревшими — рекуррентным сетям и их современным наследникам. DeepAR заложил основы вероятностного прогнозирования с RNN. TiRex и FlowState показывают, что последовательные модели в форме xLSTM и State Space Models возвращаются и конкурируют с трансформерами на современных бенчмарках.