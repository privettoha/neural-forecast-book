...
x~t=xt1T∑i=1T∣xi∣+ϵ\tilde{x}_t = \frac{x_t}{\frac{1}{T}\sum_{i=1}^{T}|x_i| + \epsilon}x~t​=T1​∑i=1T​∣xi​∣+ϵxt​​
...
После нормализации значения квантуются в один из BB B бинов. Chronos использует B=4096B = 4096 B=4096 бинов, равномерно распределённых в диапазоне, покрывающем типичные нормализованные значения.

token(x)=round(x−xmin⁡xmax⁡−xmin⁡⋅(B−1))\text{token}(x) = \text{round}\left(\frac{x - x_{\min}}{x_{\max} - x_{\min}} \cdot (B - 1)\right)token(x)=round(xmax​−xmin​x−xmin​​⋅(B−1))
