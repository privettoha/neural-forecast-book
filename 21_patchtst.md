...
Обе идеи звучат как упрощение, как отказ от выразительности. Но именно это упрощение оказывается ключом к эффективности. PatchTST показывает state-of-the-art результаты на стандартных бенчмарках, обходя и сложные трансформеры с cross-channel attention, и простые линейные модели вроде DLinear.
...
Допустим, у нас есть ряд длины L=96L = 96 L=96 (например, 96 часов истории). Мы хотим разбить его на патчи размера P=16P = 16 P=16 с шагом (stride) S=8S = 8 S=8.

Первый патч: точки 0–15 Второй патч: точки 8–23 Третий патч: точки 16–31 И так далее...

Количество патчей:

N=⌊L−PS⌋+1N = \left\lfloor \frac{L - P}{S} \right\rfloor + 1N=⌊SL−P​⌋+1

Для наших параметров: N=⌊(96−16)/8⌋+1=11N = \lfloor (96 - 16) / 8 \rfloor + 1 = 11 N=⌊(96−16)/8⌋+1=11 патчей.

Каждый патч — это вектор размерности PP P. Дальше этот вектор проецируется в пространство размерности DD D (размерность модели) через обычный линейный слой:

zi=W⋅patchi+bz_i = W \cdot \text{patch}_i + bzi​=W⋅patchi​+b

где W∈RD×PW \in \mathbb{R}^{D \times P} W∈RD×P — матрица проекции.

После проекции добавляется позиционное кодирование — чтобы модель знала, какой патч идёт раньше, какой позже:

zi′=zi+PEiz_i' = z_i + \text{PE}_izi′​=zi​+PEi​

Теперь у нас есть последовательность из NN N токенов (патчей) размерности DD D, и мы можем применить стандартный transformer encoder.
...
**Снижение сложности.** Вместо LL L токенов у нас N≈L/SN \approx L/S N≈L/S токенов. Для L=512L = 512 L=512 и S=8S = 8 S=8 это снижение с 512 до ~64 токенов. Квадратичная сложность attention падает с 5122=262144512^2 = 262144 5122=262144 до 642=409664^2 = 4096 642=4096 — в 64 раза.

**Более длинный контекст.** При фиксированном вычислительном бюджете патчинг позволяет смотреть на более длинную историю. Если мы можем обработать 64 токена, то без патчинга это 64 точки истории, а с патчингом (P=16P=16 P=16, S=8S=8 S=8) — это 512 + 16 = 528 точек при том же количестве токенов.
...
Формально: вход размера (B,C,L)(B, C, L) (B,C,L) — батч × каналы × время — преобразуется в (B⋅C,L)(B \cdot C, L) (B⋅C,L), обрабатывается как набор независимых одномерных рядов, и результат собирается обратно.
...
**Меньше параметров для обучения.** Cross-channel модель должна выучить C2C^2 C2 потенциальных взаимодействий. Для 100 каналов это 10 000 взаимодействий, многие из которых слабые или отсутствуют. Channel-independent модель фокусируется на временных паттернах внутри каждого канала — более robust задача.
...
**Transformer Encoder.** Стандартный encoder из оригинальной статьи «Attention Is All You Need»: multi-head self-attention, feed-forward network, residual connections, layer normalization. Никаких модификаций, специфичных для временных рядов — вся адаптация происходит через патчинг.

**Flatten + Linear.** После трансформера у нас NN N токенов размерности DD D. Они конкатенируются в один вектор размерности N⋅DN \cdot D N⋅D и проецируются линейным слоём в прогноз размерности HH H.

## Self-supervised pretraining

PatchTST предлагает дополнительную возможность — предобучение без разметки через masked patch prediction. Идея заимствована из BERT: случайно маскируем часть патчей и обучаем модель их восстанавливать.

**Маскирование.** Выбираем случайные r%r\% r% патчей (например, 40%) и заменяем их на специальный [MASK] токен.
...
**Channel independence как ограничение.** Для данных с сильными межканальными зависимостями (например, взаимодополняющие товары) отказ от cross-channel моделирования может стоить качества. В таких случаях TSMixer или iTransformer могут быть лучшим выбором.
